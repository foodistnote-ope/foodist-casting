            ...
            complete: (results) => {
                try {
                    ...
                    const patches: FoodistPatch[] = results.data
                        ...
                        // 会員登録状況（テンプレートに存在するが未対応だったため追加）
                        const membership = getVal('会員登録状況');
                        if (membership && ['あり', 'なし', '要確認'].includes(membership.trim())) {
                            patch.membershipStatus = membership.trim() as Foodist['membershipStatus'];
                        }
                        ...
                        if (patch.membershipStatus === 'あり') {
                            const memberTag = allTags.find(t => t.name === 'フーディスト会員');
                            if (memberTag) {
                                if (!patch.tagIds) patch.tagIds = [];
                                if (!patch.tagIds.includes(memberTag.id)) {
                                    patch.tagIds.push(memberTag.id);
                                }
                            }
                        }
